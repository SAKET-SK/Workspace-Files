...
| **`embeddedVideo`** | `true` | If set to `'true'`, YouTube and Oracle Video Hub video links can be embedded into messages so that they can be played within the chat widget. |
...
| `label` | Key of the field | string | Yes |
...
The text field inherits all read only field properties. The `displayType` value for this field is `"text"`.
...
This represents an array of fields along with a title. Used in `Table-Form` messages for nested forms of a table row.
...
Represents an Editable Form message. The message consists of an array of `fields`. It applies the following properties to the `Message`:
...
| `formColumns` | The number of columns in which the form fields should be grouped. | number | Yes |
